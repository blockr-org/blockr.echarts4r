new_e_remove_serie_p_block <- function(data, ...){
  blockr::new_block(
    name = "e_remove_serie_p_block",
    expr = quote(
      echarts4r::e_remove_serie_p(
        proxy = .(proxy)
      )
    ),
    fields = list(
    ...
    class = c("e_remove_serie_p_block", "echarts_layer_block")
  )
}

e_remove_serie_p_block <- function(data, ...){
